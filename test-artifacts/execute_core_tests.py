...
class QuorumTestExecutor:
    ...
    def execute_step1(self):
        ...
        self.log("Query agent run endpoint and monitor logs for Snapshot queries", step=1)
        ...
        self.log(f"Executing agent run with payload: {json.dumps(payload, indent=2)}", step=1)
            ...
            if os.path.exists(log_file):
                try:
                    ...
                    if 'snapshot' in content.lower():
                        ...
                        self.log(f"Snapshot log entries ({len(snapshot_lines)}):", step=1)
    ...
    def execute_step2(self):
        ...
        for log_file in log_files:
            if os.path.exists(log_file):
                try:
                    ...
                    decision_keywords = ['voting', 'decision', 'confidence', 'reasoning']
                    if any(keyword in content.lower() for keyword in decision_keywords):
                        found_ai_decision = True
                        self.log(f"✅ Found voting decision logic in {log_file}", step=2)
                        
                        # Extract decision-related lines
                        lines = content.split('\n')
                        decision_lines = [line for line in lines if any(kw in line.lower() for kw in decision_keywords)]
                        
                        self.log(f"Voting decision entries ({len(decision_lines)}):", step=2)
        ...
        if found_openrouter:
            ...
            self.log("⚠️  No OpenRouter API queries found (might be using test key)", step=2)
    ...
    def execute_step3(self):
        ...
        try:
            response = requests.get(f"{self.base_url}/agent-run/decisions?limit=5", timeout=10)
            
            if response.status_code == 200:
                decisions = response.json()
                self.log(f"✅ Retrieved agent decisions: {json.dumps(decisions, indent=2)}", step=3)
                ...
                if decision_list:
                    ...
                    self.log("⚠️  No decisions recorded yet (might be first run)", step=3)
    ...
    def execute_step4(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.log(f"Checking AttestationTracker at {self.attestation_tracker}", step=4)
                ...
                if response.status_code == 200:
                    ...
                    if 'result' in result:
                        ...
                        self.log("⚠️  Note: Actual transaction verification requires contract deployment and voting", step=4)
                        self.log("✅ STEP 4 COMPLETED (testnet ready)", step=4)
                        return True
                    else:
                        self.log(f"❌ Contract call failed: {result}", step=4)
                else:
                    self.log(f"❌ Contract call request failed: {response.status_code}", step=4)
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'result' in result:
                    ...
                    self.log("⚠️  Note: Actual attestation verification requires schema registration and voting", step=5)
                    self.log("✅ STEP 5 COMPLETED (EAS ready)", step=5)
                    return True
                else:
                    self.log(f"❌ EAS contract call failed: {result}", step=5)
            else:
                self.log(f"❌ EAS contract request failed: {response.status_code}", step=5)
