...
def main():
    ...
    success, result = run_command(
        ["npm", "--version"], frontend_dir, 30, "Checking npm availability"
    )
    ...
    success, result = run_command(
        ["npm", "install"], frontend_dir, 180, "Installing frontend dependencies"
    )
    ...
    success, result = run_command(
        ["npm", "run", "check"], frontend_dir, 120, "Running TypeScript type checking"
    )
    ...
    # Build the frontend
    success, result = run_command(
        ["npm", "run", "build"], frontend_dir, 180, "Building frontend for production"
    )
    ...
    # Run existing tests
    success, result = run_command(
        ["npm", "run", "test"], frontend_dir, 120, "Running frontend tests"
    )
