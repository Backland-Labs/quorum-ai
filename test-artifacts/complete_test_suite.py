...
class QuorumTestSuite:
            ...
            if log_file.exists():
                try:
                    ...
                    if any(
                        ...
                        for term in ["snapshot", "graphql", "proposals"]
                    ):
                        snapshot_found = True
                        self.log(
                            f"✅ Found Snapshot evidence in {log_file.name}", step=step
                        )
                        break
                except:
                    pass
    ...
    def execute_step2(self):
            ...
            if log_file.exists():
                try:
                    ...
                    if "openrouter" in content.lower():
                        ...
                        self.log(
                            f"✅ Found OpenRouter API calls in {log_file.name}", step=2
                        )
                    ...
                    if any(term in content.lower() for term in decision_terms):
                        ...
                        self.log(
                            f"✅ Found voting decision logic in {log_file.name}", step=2
                        )
        ...
        if not openrouter_found:
            self.log(
                "⚠️  No OpenRouter API calls found (might be using test key)", step=2
            )
    ...
    def execute_step3(self):
        ...
        try:
            response = requests.get(
                f"{self.base_url}/agent-run/decisions?limit=10", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self.log(
                    f"✅ Retrieved {len(decision_list)} recorded decisions", step=3
                )

                for i, decision in enumerate(decision_list[:3]):  # Show first 3
                    self.log(
                        f"Decision {i+1}: {decision.get('proposal_id')} -> {decision.get('vote')}",
                        step=3,
                    )
            ...
            if response.status_code == 200:
                ...
                contract_response = requests.post(
                    ...
                    json={
                        ...
                        "method": "eth_call",
                        "params": [
                            {"to": self.attestation_tracker, "data": "0x"},
                            "latest",
                        ],
                        "id": 2,
                    },
                ...
                if contract_response.status_code == 200:
                    ...
                    if "result" in result:
                        ...
                        self.log(
                            f"Contract address: {self.attestation_tracker}", step=4
                        )
        ...
        try:
            ...
            response = requests.post(
                ...
                json={
                    "jsonrpc": "2.0",
                    "method": "eth_call",
                    "params": [{"to": self.eas_contract, "data": "0x"}, "latest"],
                    "id": 3,
                },
                timeout=10,
            )
