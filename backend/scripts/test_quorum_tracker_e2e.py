...
async def simulate_contract_stats(
    ...
    print("Initial activity counts (simulated):")
...
async def run_agent_workflow(agent_run_service: AgentRunService, wallet_address: str):
    ...
    print("Starting agent run with config:")
...
async def verify_activity_tracking(
    quorum_tracker: QuorumTrackerService, agent_result: Any
):
    ...
    if agent_result.proposals_analyzed > 0:
        ...
        print(
            f"  - Would track {agent_result.proposals_analyzed} OPPORTUNITY_CONSIDERED activities"
        )
    ...
    if agent_result.proposals_analyzed == 0:
        ...
        print("  - Would track 1 NO_OPPORTUNITY activity")

    # Simulate final stats
    final_stats = {
        "opportunityConsidered": 5
        + (
            agent_result.proposals_analyzed
            if agent_result.proposals_analyzed > 0
            else 0
        ),
    ...
    try:
        ...
        for activity_type, name in activity_tests:
            ...
            if result.get("success"):
                ...
                print(
                    f"    Transaction would be sent to Safe: {result.get('tx_hash', 'pending')}"
                )
            else:
...
async def main():
    ...
    try:
        ...
        if success and direct_test_success:
            ...
            print("  - QuorumTracker integration verified")
            print("  - Direct activity registration tested")
            print("\nActivity tracking:")
            print(f"  - Contract address: {QUORUM_TRACKER_ADDRESS}")
            print("  - All activity types can be registered")
            print("  - Integration with agent run service confirmed")
        else:
