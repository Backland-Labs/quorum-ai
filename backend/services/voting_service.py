...
class VotingService:
    ...
    async def vote_on_proposal(
        ...
        """
        with logfire.span("voting_service.vote_on_proposal", space=space, proposal=proposal, choice=choice):
            logfire.info(f"Starting vote on proposal {proposal} in space {space}")
            
            # Create vote message
            vote_message = self.create_snapshot_vote_message(space, proposal, choice, timestamp)
            
            # Sign message
            signature = self.sign_snapshot_message(vote_message)
            
            # Submit to Snapshot
            submission_result = await self.submit_vote_to_snapshot(vote_message, signature)
            ...
            if submission_result["success"]:
                ...
                logfire.error("Vote workflow failed", error=submission_result.get("error"))
