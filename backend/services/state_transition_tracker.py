...
class StateTransitionTracker:
        ...
        if enable_pearl_logging:
            try:
                ...
                self.pearl_logger.log_state_transition = (
                    self._log_state_transition_fallback
                )
    ...
    def record_transition(
        ...
        with self._lock:
            ...
            if (
                ...
                and len(self.transition_history) > self.max_history_size
            ):
                self.transition_history = self.transition_history[
                    -self.max_history_size :
                ]
    ...
    def get_recent_transitions(self, seconds: float) -> list[StateTransition]:
        ...
        return [t for t in self.transition_history if t.timestamp >= cutoff_time]
    ...
    async def _migrate_from_file_storage(self):
        ...
        try:
            ...
            if self.pearl_logger:
                self.pearl_logger.info(
                    f"Migrated state from {self.state_file_path} to StateManager"
                )
        except Exception as e:
            if self.pearl_logger:
