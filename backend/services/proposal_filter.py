...
class ProposalFilter:
    ...
    def __init__(self, preferences: UserPreferences) -> None:
        ...
        assert isinstance(preferences, UserPreferences), "Preferences must be UserPreferences instance"
    ...
    def filter_proposals(self, proposals: List[Proposal]) -> List[Proposal]:
        ...
        assert all(isinstance(p, Proposal) for p in proposals), "All proposals must be Proposal objects"
        ...
        with logfire.span("filter_proposals", proposal_count=len(proposals)):
            ...
            for proposal in proposals:
                ...
                assert isinstance(proposal, Proposal), f"Invalid proposal object: {type(proposal)}"
            ...
            logfire.info(
                ...
                whitelist_filtered_count=len(proposals) - len(filtered_proposals) - blacklisted_count
            )
            
            # Runtime assertion: validate output
            assert isinstance(filtered_proposals, list), "Filtered proposals must be a list"
            assert all(isinstance(p, Proposal) for p in filtered_proposals), "All filtered proposals must be Proposal objects"
    ...
    def rank_proposals(self, proposals: List[Proposal]) -> List[Proposal]:
        ...
        with logfire.span("rank_proposals", proposal_count=len(proposals)):
            ...
            assert all(isinstance(p, Proposal) for p in ranked_proposals), "All ranked proposals must be Proposal objects"
            assert len(ranked_proposals) == len(proposals), "Ranking must preserve proposal count"
    ...
    def calculate_proposal_score(self, proposal: Proposal) -> float:
        ...
        with logfire.span("calculate_proposal_score", proposal_id=proposal.id):
            ...
            voting_power_factor = self._calculate_voting_power_factor(proposal.scores_total)
    ...
    def _calculate_urgency_factor(self, time_until_deadline: int) -> float:
        ...
        assert isinstance(time_until_deadline, int), "Time until deadline must be integer"
    ...
    def _calculate_voting_power_factor(self, scores_total: float) -> float:
        ...
        # Runtime assertion: validate output
        assert isinstance(voting_power_factor, float), "Voting power factor must be float"
        assert 0.0 <= voting_power_factor <= 1.0, "Voting power factor must be between 0 and 1"
    ...
    def _calculate_participation_factor(self, votes: int) -> float:
        ...
        # Runtime assertion: validate output
        assert isinstance(participation_factor, float), "Participation factor must be float"
        assert 0.0 <= participation_factor <= 1.0, "Participation factor must be between 0 and 1"
        
        return participation_factor
    
    def get_filtering_metrics(self, original_proposals: List[Proposal], 
                            filtered_proposals: List[Proposal]) -> Dict[str, Any]:
        ...
        for proposal in original_proposals:
            if proposal.author in self.preferences.blacklisted_proposers:
                blacklisted_count += 1
            elif self.preferences.whitelisted_proposers and proposal.author not in self.preferences.whitelisted_proposers:
        ...
        metrics = {
            ...
            "filter_efficiency": filtered_count / original_count if original_count > 0 else 0.0,
        ...
        assert all(isinstance(k, str) for k in metrics.keys()), "All metric keys must be strings"
