...
class AgentRunLogger:
    ...
    def __init__(self, log_file_path: Optional[str] = None, store_path: Optional[str] = None):
    ...
    def log_proposals_fetched(
        ...
        message = self._format_structured_message("Proposals fetched and filtered", params)
        self.logger.info(message)
    ...
    def log_proposal_analysis(self, proposal: Proposal, decision: VoteDecision) -> None:
        ...
        risk_level = decision.risk_assessment.value if decision.risk_assessment else None
    ...
    def log_vote_execution(
        ...
        if success:
            message = self._format_structured_message("Vote executed successfully", params)
            self.logger.info(message)
        else:
            params["error"] = error
    ...
    def log_agent_completion(self, response: AgentRunResponse) -> None:
        ...
        params = {
            ...
            "successful_votes": len(response.votes_cast),  # All votes in votes_cast are successful
    ...
    def log_error(self, operation: str, error: Exception, **context) -> None:
        ...
        message = self._format_structured_message(f"Agent run {operation} failed", params)
        self.logger.error(message)
