...
class TestChainConfiguration:
    ...
    def test_validate_chain_configuration(self):
        ...
        assert (
            validation["safe_address"] == "0x1234567890123456789012345678901234567890"
        )
...
class TestWeb3Connection:
    ...
    def test_get_web3_connection_no_rpc(self):
        ...
        with pytest.raises(
            ValueError, match="No RPC endpoint configured for chain: unknown"
        ):
    ...
    def test_get_web3_connection_failed(self, mock_web3_class):
        ...
        with patch.object(self.service, "_rate_limit_base_rpc"):
            with pytest.raises(
                ConnectionError, match="Failed to connect to base network"
            ):
...
class TestChainSelection:
    ...
    def test_select_optimal_chain_no_valid_chains(self):
        ...
        with (
            patch.object(self.service, "get_supported_chains", return_value=[]),
...
class TestSafeTransactionBuilding:
    ...
    async def test_build_safe_transaction_no_safe_address(self):
        ...
        with pytest.raises(
            ValueError, match="No Safe address configured for chain: unknown"
        ):
...
class TestSafeTransactionSubmission:
    ...
    async def test_submit_safe_transaction_invalid_inputs(self):
        ...
        with pytest.raises(
            AssertionError, match="To address must be a non-empty string"
        ):
            await self.service._submit_safe_transaction(
        ...
        # Test invalid value
        with pytest.raises(
            AssertionError, match="Value must be a non-negative integer"
        ):
            await self.service._submit_safe_transaction(
                chain="base",
                to="0x4567890123456789012345678901234567890123",
    ...
    async def test_submit_safe_transaction_success(
        self, mock_tx_service, mock_safe_class, mock_eth_client
    ):
        ...
        with (
            patch.object(self.service, "get_web3_connection", return_value=mock_w3),
    ...
    @patch("services.safe_service.Safe")
    @patch("services.safe_service.TransactionServiceApi")
    async def test_submit_safe_transaction_simulation_failure(
        self, mock_tx_service, mock_safe_class, mock_eth_client
    ):
...
class TestActivityTransaction:
    ...
    async def test_perform_activity_transaction_auto_chain(self):
        ...
        with (
            patch.object(self.service, "select_optimal_chain", return_value="base"),
            patch.object(
                self.service, "_submit_safe_transaction", new_callable=AsyncMock
            ) as mock_submit,
        ):
...
class TestEASAttestation:
    ...
    def setup_method(self):
        ...
        with (
            ...
            mock_settings.gnosis_ledger_rpc = "https://gnosis-rpc.com"
            mock_settings.mode_ledger_rpc = "https://mode-rpc.com"
            mock_settings.eas_contract_address = (
    ...
    async def test_create_eas_attestation_success(
        self, mock_load_abi, mock_get_w3, mock_settings
    ):
        ...
        with (
            patch.object(self.service, "_build_eas_attestation_tx") as mock_build_tx,
            patch.object(
                self.service, "_submit_safe_transaction", new_callable=AsyncMock
            ) as mock_submit,
        ):
    ...
    @patch("services.safe_service.settings")
    @patch("builtins.open", new_callable=mock_open, read_data="0x" + "a" * 64)
    def test_build_delegated_attestation_tx_eip712proxy(
        self, mock_file, mock_settings, mock_load_abi, mock_get_w3, mock_generate_sig
    ):
    ...
    @patch("services.safe_service.settings")
    @patch("builtins.open", new_callable=mock_open, read_data="0x" + "a" * 64)
    def test_build_delegated_attestation_tx_attestation_tracker(
        self, mock_file, mock_settings, mock_load_abi, mock_get_w3, mock_generate_sig
    ):
...
class TestSafeTransactionSubmissionComprehensive:
    ...
    @patch("services.safe_service.Safe")
    @patch("services.safe_service.TransactionServiceApi")
    async def test_submit_safe_transaction_complete_flow(
        self, mock_tx_service, mock_safe_class, mock_eth_client
    ):
    ...
    @patch("services.safe_service.Safe")
    @patch("services.safe_service.TransactionServiceApi")
    async def test_submit_safe_transaction_reverted_receipt(
        self, mock_tx_service, mock_safe_class, mock_eth_client
    ):
    ...
    @patch("services.safe_service.EthereumClient")
    @patch("services.safe_service.Safe")
    async def test_submit_safe_transaction_exception_handling(
        self, mock_safe_class, mock_eth_client
    ):
...
class TestSafeTransactionBuildingComprehensive:
    ...
    @patch("services.safe_service.EthereumClient")
    @patch("services.safe_service.Safe")
    async def test_build_safe_transaction_with_all_params(
        self, mock_safe_class, mock_eth_client
    ):
...
class TestEASAttestationComprehensive:
    ...
    async def test_create_eas_attestation_with_attestation_tracker(self):
        ...
        with patch("services.safe_service.settings") as mock_settings:
            mock_settings.attestation_tracker_address = (
                "0xtracker567890123456789012345678901234567890"
            )
            mock_settings.eas_contract_address = "0x12345678901234567890123456789012345678904567890123456789012345678901234567890"
            ...
            with (
                patch.object(
                    self.service, "_build_eas_attestation_tx"
                ) as mock_build_tx,
                patch.object(
                    self.service, "_submit_safe_transaction", new_callable=AsyncMock
                ) as mock_submit,
...
class TestUtilityMethodsComprehensive:
    ...
    def test_generate_eas_delegated_signature_comprehensive(
        ...
        result = self.service._generate_eas_delegated_signature(
            request_data,
            mock_w3,
            "0x12345678901234567890123456789012345678904567890123456789012345678901234567890",
        )
...
class TestEdgeCasesAndErrorHandling:
    ...
    @patch("services.safe_service.EthereumClient")
    @patch("services.safe_service.Safe")
    async def test_build_safe_transaction_empty_data_handling(
        self, mock_safe_class, mock_eth_client
    ):
...
class TestSafeServiceMissingCoverage:
    ...
    @patch("services.safe_service.Safe")
    @patch("services.safe_service.TransactionServiceApi")
    async def test_submit_safe_transaction_no_safe_address_configured(
        self, mock_tx_service, mock_safe_class, mock_eth_client
    ):
        ...
        with (
            patch.object(self.service, "safe_addresses", {"base": None}),
            ...
            mock_safe_class.side_effect = ValueError(
                "No Safe address configured for chain: base"
            )
    ...
    @patch("services.safe_service.EthereumClient")
    @patch("services.safe_service.Safe")
    async def test_build_safe_transaction_with_none_data(
        self, mock_safe_class, mock_eth_client
    ):
    ...
    def test_build_delegated_attestation_tx_with_env_private_key(self):
        ...
        with (
            ...
            patch("services.safe_service.settings") as mock_settings,
            patch.object(
                self.service, "_generate_eas_delegated_signature"
    ...
    def test_build_delegated_attestation_tx_no_private_key(self):
        ...
        with (
            ...
            patch.object(
                self.service,
                "_generate_eas_delegated_signature",
                return_value=b"\x01" * 65,
            ),
            patch("builtins.open", side_effect=FileNotFoundError),
