...
class APITester:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if "status" in data and data["status"] == "healthy":
                    self.log_result("Health Check", True, f"Status: {data['status']}", response_time)
                    return True
                else:
                    self.log_result("Health Check", False, f"Invalid response: {data}", response_time)
                    return False
            else:
            ...
            if response.status_code == 200:
                ...
                if "proposals" in data and isinstance(data["proposals"], list):
                    ...
                    if proposal_count > 0:
                        ...
                        self.log_result("Proposals List", True, f"Found {proposal_count} proposals", response_time)
                        return proposal_id
                    else:
                        self.log_result("Proposals List", True, "No proposals found (valid response)", response_time)
                        return None
                else:
                    self.log_result("Proposals List", False, f"Invalid response structure: {data}", response_time)
                    return None
            else:
                self.log_result("Proposals List", False, f"HTTP {response.status_code}: {response.text}", response_time)
                return None
    ...
    def test_proposal_by_id(self, proposal_id: str) -> bool:
        ...
        try:
            start = time.time()
            response = requests.get(f"{self.base_url}/proposals/{proposal_id}", timeout=TIMEOUT)
            response_time = time.time() - start
            ...
            if response.status_code == 200:
                ...
                if all(field in data for field in required_fields):
                    self.log_result("Proposal by ID", True, f"ID: {data['id'][:20]}..., Title: {data['title'][:50]}...", response_time)
                    return True
                else:
                    missing = [f for f in required_fields if f not in data]
                    self.log_result("Proposal by ID", False, f"Missing fields: {missing}", response_time)
                    return False
            else:
                self.log_result("Proposal by ID", False, f"HTTP {response.status_code}: {response.text}", response_time)
                return False
            ...
            if response.status_code == 200:
                ...
                if "summaries" in data and len(data["summaries"]) > 0:
                    ...
                    if "summary" in summary and "proposal_id" in summary:
                        self.log_result("AI Summarization", True, f"Generated summary for {summary['proposal_id']}", response_time)
                        return True
                    else:
                        self.log_result("AI Summarization", False, f"Invalid summary structure: {summary}", response_time)
                        return False
                else:
                    self.log_result("AI Summarization", False, f"No summaries returned: {data}", response_time)
                    return False
            else:
                self.log_result("AI Summarization", False, f"HTTP {response.status_code}: {response.text}", response_time)
                return False
    ...
    def test_top_voters(self, proposal_id: str) -> bool:
        ...
        try:
            start = time.time()
            response = requests.get(f"{self.base_url}/proposals/{proposal_id}/top-voters", timeout=TIMEOUT)
            response_time = time.time() - start
            ...
            if response.status_code == 200:
                ...
                if "proposal_id" in data and "voters" in data:
                    ...
                    self.log_result("Top Voters", True, f"Found {voter_count} voters for proposal", response_time)
                    return True
                else:
                    self.log_result("Top Voters", False, f"Invalid response structure: {data}", response_time)
                    return False
            else:
                self.log_result("Top Voters", False, f"HTTP {response.status_code}: {response.text}", response_time)
                return False
