...
class TestProposalTopVotersEndpoint:
    ...
    def test_limit_parameter_custom(self, client: TestClient, sample_voters):
        ...
        # Act
        response = client.get(f"/proposals/{proposal_id}/top-voters?limit={custom_limit}")
        
        # Assert
        assert response.status_code == 200
        # Verify the service was called with custom limit
        main.tally_service.get_proposal_votes.assert_called_once_with(proposal_id, custom_limit)
    ...
    def test_caching_headers_completed_proposal(self, client: TestClient, sample_voters):
        ...
        main.tally_service.get_proposal_by_id = AsyncMock(return_value=completed_proposal)
