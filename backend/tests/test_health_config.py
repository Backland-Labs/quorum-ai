...
class TestHealthExceptions:
    ...
    def test_health_service_timeout_error_creation(self):
        ...
        error = HealthServiceTimeoutError(
            error_message, timeout_ms=timeout_ms, operation=operation
        )

        assert isinstance(
            error, HealthCheckError
        )  # Should inherit from HealthCheckError
        assert str(error) == error_message
    ...
    def test_exception_string_representation(self):
        ...
        timeout_error = HealthServiceTimeoutError(
            "Timeout occurred", timeout_ms=50, operation="test"
        )
