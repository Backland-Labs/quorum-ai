...
class TestCacheServiceInitialization:
    ...
    async def test_successful_initialization(self, cache_service, mock_redis):
        ...
        mock_pool = AsyncMock()
        with patch("services.cache_service.ConnectionPool.from_url", return_value=mock_pool):
    ...
    async def test_initialization_connection_error(self, cache_service):
        ...
        with patch("services.cache_service.ConnectionPool.from_url", return_value=mock_pool):
            ...
            mock_redis_failing.ping = AsyncMock(side_effect=ConnectionError("Connection failed"))
