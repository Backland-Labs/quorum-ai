...
from models import (
    Proposal, ProposalVoter, Space,
    UserPreferences, VotingStrategy, VoteType,
    SummarizeResponse, AiVoteResponse
)
...
class AgentTestFixtures:
    ...
    def create_sample_proposals_batch(count: int = 5) -> List[Proposal]:
        ...
        proposal_types = [
            ("Treasury Management", "Allocate funds for development", VotingStrategy.CONSERVATIVE),
            ...
            ("Governance Change", "Update voting parameters", VotingStrategy.CONSERVATIVE),
    ...
    def create_sample_voters(count: int = 5) -> List[ProposalVoter]:
        ...
        for i in range(count):
            voters.append(ProposalVoter(
                ...
                choice="For" if i % 3 == 0 else "Against" if i % 3 == 1 else "Abstain"
            ))
    ...
    def create_sample_ai_vote_response(
        decision: VoteType = VoteType.FOR,
    ...
    def create_edge_case_proposals() -> List[Proposal]:
        ...
        return [
            ...
            AgentTestFixtures.create_sample_proposal(
                proposal_id="edge-empty-body",
                title="Proposal with Empty Body",
                body=""
            ),
        ...
        return {
            "query_active_proposals": [
                ...
                },
                {
                    ...
                    "state": "active",
                    "end_timestamp": int((datetime.now(timezone.utc) + timedelta(days=1)).timestamp()),
