...
def execute_attestation(
    ...
    """Execute attestation through AttestationTracker as if from Safe."""
    
    print_header("STEP 2: Execute Attestation (Simulating SafeService)")
    
    # Encode attestation data (simplified for testing)
    encoded_data = w3.codec.encode(
        ['string', 'bool'],
        [f'Test from {attestation_data.space_id}', True]
    )
    ...
    sig_data = generate_eas_signature(
        w3, 
        encoded_data,
    ...
    print_detail("Signature generated by", sig_data['signer'])
    ...
    # Check which interface the contract uses
    has_fixed_interface = False
    for func in tracker_contract.abi:
        if func.get('name') == 'attestByDelegation' and len(func.get('inputs', [])) == 12:
            has_fixed_interface = True
            break
    ...
    try:
        if has_fixed_interface:
            ...
            # Call with 12 parameters (ensure addresses are checksummed)
            tx_hash = tracker_contract.functions.attestByDelegation(
                schema_bytes,                           # schema
                Web3.to_checksum_address(attestation_data.agent),  # recipient (checksummed)
                0,                                       # expirationTime
                True,                                    # revocable
                bytes(32),                              # refUID
                encoded_data,                           # data
                0,                                      # value
                sig_data['v'],                          # signature v
                sig_data['r'],                          # signature r
                sig_data['s'],                          # signature s
                Web3.to_checksum_address(sig_data['signer']),  # attester (checksummed)
                deadline                                # deadline
            ).transact({'from': safe_address, 'gas': 500000})
        else:
            # Use original 4-parameter interface
            print_info("Using 4-parameter interface (original AttestationTracker)")
            ...
            # Build nested structure for original interface (with checksummed addresses)
            delegated_request = (
                schema_bytes,
                (
                    Web3.to_checksum_address(attestation_data.agent),  # recipient (checksummed)
        ...
        print_error(f"Attestation failed: {str(e)}")
